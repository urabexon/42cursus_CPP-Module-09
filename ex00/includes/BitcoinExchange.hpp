...
/*   Updated: 2025/04/02 14:28:05 by urabex           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/*
  ビットコインの履歴データ(data.csv)は日付とレートのペアで構成されている。
  任意の日付をキーにして、そのレートを素早く取得する必要がある。
  mapの特性上、キー(日付)に対して常にソート済みで格納される。
  lower_boundなどを使用すれば指定した日付で最も近いレートを対数時間で検索できる。
  これが「該当する日付が存在しない場合、直近過去のレートを使う」という要件に対して適しているためmapで実装
*/
